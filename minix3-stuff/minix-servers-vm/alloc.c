...
//commentary added is that this do section may well be the first fit alg
  do {
	mem = alloc_pages(clicks, memflags);
  } while(mem == NO_MEM && cache_freepages(clicks) > 0);
//commentary added is that the if below is the Error state
  if(mem == NO_MEM)
  	return mem;
//commentary added that the if below shifts memory block to be on a click boundry?
  if(align_clicks) {
  	phys_clicks o;
  	o = mem % align_clicks;
  	if(o > 0) {
...
static int findbit(int low, int startscan, int pages, int memflags, int *len)
{
	...
	// start at startscan and go backwards until you hit low
	for(i = startscan; i >= low; i--) {
		// if page at address is free
		if(!page_isfree(i)) {
			int pi;
...
// Best Fit 
static int findbitBestFit(int low, int startscan, int pages, int memflags, int *len)
{
	int run_length = 0, i;
	int freerange_start = startscan;
	//this sets bestSize to the maximum value and int can be
	...
	for(i = startscan; i >= low; i--) {
		...
		//checks if the next page is free and if the block is big enough but smaller than the current best
		if((!page_isfree(i)) && (page_isfree(i-1)) && (run_length >= pages) && (run_length < bestSize)){
			//if the above is true then we need to change what the bestSize is and what the bestAddress is
			//bestSize is the size of the what the current block and that's the new best
			bestSize = run_length;
			//record the new start
	...
	// assert(run_length <= pages);
	if(bestSize >= pages && bestSize < INT_MAX) {
		/* good block found! */
		*len = pages;
		return bestAddress;
	}
...
// Worst Fit
static int findbitWorstFit(int low, int startscan, int pages, int memflags, int *len)
{
	int run_length = 0, i;
	int freerange_start = startscan;
	//this sets worstSize to 0 because nothing should be able to fit there
	int worstSize = 0;
	int worstAddress;
	...
	for(i = startscan; i >= low; i--) {
		...
		//checks if the next page is free and if the block is big enough and bigger than the current worst
		if((!page_isfree(i)) && (page_isfree(i-1)) && (run_length >= pages) && (run_length > worstSize)){
			//if the above is true then we need to change what the worstSize is and what the worstAddress is
			//worstSize is the size of the what the current block and that's the new worst
			worstSize = run_length;
			//record the new start
...
// Random Fit
static int findbitRandomFit(int low, int startscan, int pages, int memflags, int *len)
{
	int run_length = 0, i;
	int freerange_start = startscan;
	int addressList[500];
	...
	for(i = startscan; i >= low; i--) {
		...
		//checks if the next page is free and if the block is big enough
		if((!page_isfree(i)) && (page_isfree(i-1)) && (run_length >= pages)){
			//populate address list to have choices to choose from for random fit
	...
	if(addressNumber >= 1) {
		/* good block found! */
		*len = pages;
		int addressChoosen = addressList[randomPick % addressNumber];
...
static phys_bytes alloc_pages(int pages, int memflags)
{
	...
	//if else if else statement to choose which aglorithm to use
	if(chosenAlgorithm == firstFit){
		if(mem == NO_MEM)
	...
	else if (chosenAlgorithm == bestFit){
		if(mem == NO_MEM)
			mem = findbitBestFit(0, startscan, pages, memflags, &run_length);
		if(mem == NO_MEM)
			mem = findbitBestFit(0, maxpage, pages, memflags, &run_length);
		if(mem == NO_MEM)
			return NO_MEM;
	}
	else if (chosenAlgorithm == worstFit){
		if(mem == NO_MEM)
			mem = findbitWorstFit(0, startscan, pages, memflags, &run_length);
		if(mem == NO_MEM)
			mem = findbitWorstFit(0, maxpage, pages, memflags, &run_length);
		if(mem == NO_MEM)
			return NO_MEM;
	}
	else if (chosenAlgorithm == randomFit){
		if(mem == NO_MEM)
			mem = findbitRandomFit(0, startscan, pages, memflags, &run_length);
		if(mem == NO_MEM)
			mem = findbitRandomFit(0, maxpage, pages, memflags, &run_length);
		if(mem == NO_MEM)
			return NO_MEM;
	}
	else{
